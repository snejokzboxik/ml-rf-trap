...
def create_fem_connections_figure(
    ...
    """Plot mesh, electric potential, and ``|E|^2`` for both connections."""

    common_view_half_width_mm = 14.0
    figure, axes = plt.subplots(2, 3, figsize=(14.8, 8.25))
    _plot_mesh_panel(
        axes[0, 0],
        comparison.trap_mesh,
        comparison.quadrupole.geometry,
        comparison.quadrupole.potentials_v,
        "(a) Quadrupole: FEM mesh",
        view_half_width_mm=common_view_half_width_mm,
    )
    _plot_electric_potential_panel(
        axes[0, 1],
        comparison.trap_mesh,
        comparison.quadrupole,
        "(b) Quadrupole: electric potential",
        view_half_width_mm=common_view_half_width_mm,
    )
    _plot_effective_potential_panel(
        ...
        comparison.trap_mesh,
        comparison.quadrupole,
        "(c) Quadrupole: effective potential",
        view_half_width_mm=common_view_half_width_mm,
        scale_percentiles=(0.2, 95.0),
        contour_level_count=12,
        minimum_marker_size=62.0,
    )
    _plot_mesh_panel(
        axes[1, 0],
        comparison.trap_mesh,
    ...
    _plot_effective_potential_panel(
        ...
        comparison.trap_mesh,
        comparison.in_phase,
        "(f) In-phase: effective potential",
        view_half_width_mm=common_view_half_width_mm,
        scale_percentiles=(0.1, 75.0),
        contour_level_count=24,
        minimum_marker_size=96.0,
    )
    figure.suptitle(
        f"Real 2D FEM fields for one displaced geometry "
    ...
    figure.text(
        0.5,
        0.012,
        "Same displaced geometry and common +/-14 mm view in every panel: "
        "3,996 nodes, 7,384 triangles, 500 µm central refinement",
        ha="center",
        va="bottom",
        fontsize=8,
        color=INK,
...
def _plot_mesh_panel(
    ...
    *,
    ...
    _draw_electrode_holes(
        axis,
        geometry,
        potentials_v,
        view_half_width_mm=view_half_width_mm,
    )
    _format_geometry_axis(axis, title, view_half_width_mm=view_half_width_mm)
    axis.text(
        0.02,
        0.02,
...
def _plot_effective_potential_panel(
    ...
    *,
    view_half_width_mm: float = 27.0,
    scale_percentiles: tuple[float, float] = (0.5, 99.5),
    contour_level_count: int = 0,
    minimum_marker_size: float = 36.0,
) -> None:
    ...
    crop_mask = (
        (np.abs(points_mm[0]) <= view_half_width_mm)
        & (np.abs(points_mm[1]) <= view_half_width_mm)
        & (psi_nodes > 0.0)
    )
    ...
    lower = max(
        float(np.percentile(crop_values, scale_percentiles[0])),
        np.finfo(float).tiny,
    )
    upper = max(
        float(np.percentile(crop_values, scale_percentiles[1])),
        10.0 * lower,
    )
    ...
    if contour_level_count > 0:
        normalized_positions = np.linspace(0.0, 1.0, contour_level_count) ** 1.6
        contour_levels = np.exp(
            np.log(lower) + normalized_positions * np.log(upper / lower)
        )
        axis.tricontour(
            points_mm[0],
            points_mm[1],
    ...
    _format_geometry_axis(axis, title, view_half_width_mm=view_half_width_mm)
    if view_half_width_mm < 27.0:
        axis.text(
            0.02,
            0.98,
            f"Common view: ±{view_half_width_mm:g} mm",
            transform=axis.transAxes,
            ha="left",
            va="top",
            fontsize=6.5,
            color=INK,
            bbox={"facecolor": WHITE, "edgecolor": GREY_LIGHT, "pad": 2.2},
            zorder=10,
        )
...
def _soft_colormap(
    name: str,
    *,
    lower_fraction: float = 0.10,
    upper_fraction: float = 0.90,
) -> mpl.colors.Colormap:
    """Return a restrained version of a Matplotlib colormap."""

    base = mpl.colormaps[name]
    colors = base(np.linspace(lower_fraction, upper_fraction, 256))
...
def _electric_potential_plot_style(
    trap_mesh: TrapMesh,
    connection: ConnectionResult,
) -> tuple[Normalize, mpl.colors.Colormap, tuple[float, float]]:
    """Return a central-contrast normalization for the real FEM potential."""

    points_mm = 1.0e3 * trap_mesh.mesh.p
    potential_v = np.asarray(connection.solution.potential_v, dtype=float)
    central_mask = (
        (np.abs(points_mm[0]) <= 8.0)
        & (np.abs(points_mm[1]) <= 8.0)
        & np.isfinite(potential_v)
    )
    central_values = potential_v[central_mask]
    if central_values.size == 0:
        raise RuntimeError("electric-potential central crop contains no FEM nodes")

    electrode_values = np.asarray(connection.potentials_v, dtype=float)
    has_both_signs = bool(
        np.any(electrode_values < 0.0) and np.any(electrode_values > 0.0)
    )
    if has_both_signs:
        limit = max(
            float(np.percentile(np.abs(central_values), 97.5)),
            1.0e-6,
        )
        return (
            TwoSlopeNorm(vmin=-limit, vcenter=0.0, vmax=limit),
            _soft_colormap("coolwarm", lower_fraction=0.08, upper_fraction=0.92),
            (-limit, limit),
        )

    lower, upper = np.percentile(central_values, (3.0, 97.0))
    lower = float(lower)
    upper = float(upper)
    span_floor = max(1.0e-6, 1.0e-4 * max(abs(lower), abs(upper), 1.0))
    if upper - lower < span_floor:
        midpoint = 0.5 * (lower + upper)
        lower = midpoint - 0.5 * span_floor
        upper = midpoint + 0.5 * span_floor
    return (
        Normalize(vmin=lower, vmax=upper),
        _soft_colormap("cividis", lower_fraction=0.08, upper_fraction=0.92),
        (lower, upper),
    )


def _plot_electric_potential_panel(
    axis: mpl.axes.Axes,
    trap_mesh: TrapMesh,
    connection: ConnectionResult,
    title: str,
    *,
    ...
    """Plot the real scalar FEM potential with a central-contrast color scale."""

    points_mm = 1.0e3 * trap_mesh.mesh.p
    potential_v = np.asarray(connection.solution.potential_v, dtype=float)
    norm, cmap, limits = _electric_potential_plot_style(trap_mesh, connection)
    color_mesh = axis.tripcolor(
        points_mm[0],
        points_mm[1],
        trap_mesh.mesh.t.T,
    ...
    axis.text(
        0.02,
        0.02,
        "Central-region contrast\n"
        f"{limits[0]:.4g} to {limits[1]:.4g} V",
        transform=axis.transAxes,
        ha="left",
        va="bottom",
        fontsize=6.5,
    ...
    colorbar.set_label(r"Electric potential $\phi$ (V)", fontsize=8)
    colorbar.ax.tick_params(labelsize=7)
...
def _draw_electrode_holes(
    ...
    *,
    ...
    for center_m, potential_v in zip(
        ...
        label_limit = max(view_half_width_mm - 0.8, 0.0)
        label_position_mm = np.clip(center_mm, -label_limit, label_limit)
        axis.add_patch(
            Circle(
                center_mm,
                radius_mm,
...
def _format_geometry_axis(
    ...
    *,
    ...
    axis.set_xlim(-view_half_width_mm, view_half_width_mm)
    axis.set_ylim(-view_half_width_mm, view_half_width_mm)
    axis.set_aspect("equal")
...
def write_fem_metadata(
    ...
    quadrupole_potential_limits = _electric_potential_plot_style(
        comparison.trap_mesh,
        comparison.quadrupole,
    )[2]
    in_phase_potential_limits = _electric_potential_plot_style(
        comparison.trap_mesh,
        comparison.in_phase,
    )[2]
    ...
    for path in output_paths:
        try:
            relative_output_paths.append(path.relative_to(Path.cwd()).as_posix())
        except ValueError:
            relative_output_paths.append(path.as_posix())
    metadata = {
        "coordinate_units": "metres",
        "electric_potential": {
            "quantity": "FEM scalar potential phi",
            "units": "V",
            "visualization_scaling": (
                "restrained central-region contrast: quadrupole uses the "
                "97.5th absolute percentile symmetrically about zero; "
                "in-phase uses the 3rd-to-97th percentiles"
            ),
            "central_region_half_width_m": 8.0e-3,
            "quadrupole_color_limits_v": list(quadrupole_potential_limits),
            "in_phase_color_limits_v": list(in_phase_potential_limits),
        },
        "effective_potential_proxy": "Psi = |E|^2 in V^2/m^2",
        "effective_potential_visualization": {
            "normalization": "logarithmic, using real recovered-field values",
            "quadrupole_display_percentiles": [0.2, 95.0],
            "quadrupole_contour_count": 12,
            "in_phase_display_percentiles": [0.1, 75.0],
            "in_phase_contour_count": 24,
            "contour_spacing": "logarithmic with increased density near wells",
        },
        "figure_panels": [
            "quadrupole FEM mesh",
            "quadrupole electric potential phi",
            "quadrupole effective-potential proxy Psi",
            "in-phase FEM mesh",
            "in-phase electric potential phi",
            "in-phase effective-potential proxy Psi",
        ],
        "panel_view_half_widths_mm": {
            "quadrupole_mesh": 14.0,
            "quadrupole_electric_potential": 14.0,
            "quadrupole_effective_potential": 14.0,
            "in_phase_mesh": 14.0,
            "in_phase_electric_potential": 14.0,
            "in_phase_effective_potential": 14.0,
        },
        "fem_electrode_order": [
            "F1 upper-left",
            "F2 upper-right",
...
def write_readme(
    ...
    """Document provenance, figure roles, notes, and article captions."""
...
def build_all_figures(repo_root: Path) -> dict[str, object]:
    ...
    dataset_path = (
        repo_root
        / "validation_results"
        / "article_fem_sample_51974"
        / "synthetic_clean_ml_row0.csv"
    )
