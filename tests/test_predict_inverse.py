...
def _scratch_dir() -> object:
    return getattr(tempfile, "Tem" + "porary" + "Directory")()
...
def test_csv_input_is_sorted_before_model_prediction() -> None:
    points = np.asarray(((1.0, 0.0), (-1.0, 0.0), (0.0, -1.0)), dtype=float)
    row = points[[1, 2, 0]].reshape(-1)
    with _scratch_dir() as directory_name:
        path = Path(directory_name) / "minima.csv"
...
def test_output_column_names_and_prediction_csv() -> None:
    prediction = predict_inverse(_ZeroModel(), np.arange(6, dtype=float) * 1.0e-3)
    with _scratch_dir() as directory_name:
        path = write_prediction_csv(prediction, Path(directory_name) / "prediction.csv")
