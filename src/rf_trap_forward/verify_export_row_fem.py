...
This check deliberately reuses the practical synthetic-data worker. It
...
def wrong_direct_fem_mapping(true_wolfram_m: object) -> NDArray[np.float64]:
    """Return the intentionally wrong comparison mapping W1..W4 -> F1..F4."""
...
) -> tuple[MappingCheck, MappingCheck]:
...
def _summary_row(row: ExportRow, result: MappingCheck) -> dict[str, object]:
...
def diagnose_export_rows(
    ...
    *,
    row_indices: Sequence[int] = (),
    sample_ids: Sequence[int] = (),
    output_dir: str | Path = "validation_results/export_row_fem_check",
) -> tuple[Path, ...]:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("validation_results/export_row_fem_check")
    )
    return parser
